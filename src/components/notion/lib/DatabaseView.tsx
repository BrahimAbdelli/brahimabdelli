...
const ArticleSummary: React.FC<ArticleSummaryProps> = ({ article }) => {
  ...
  const descriptionText = richTextToPlainText(properties?.description?.rich_text);

  const parentDatabaseId = article?.parent?.database_id?.replace(/-/g, '');

  const href =
    parentDatabaseId === siteConfig.notion.baseBlock
      ? `/${encodeURIComponent(slug)}`
      : `/${encodeURIComponent(id.replace(/-/g, ''))}/${encodeURIComponent(slug || 'Untitled')}`;
    ...
    >
      <div className='w-full flex flex-col bg-base-content/5 shadow-md overflow-hidden rounded-md isolate sm:flex-row'>
        ...
        <div className='flex-auto flex flex-col justify-between p-4 py-3 sm:py-2'>
          ...
          {descriptionText && (
            <div className='text-xs text-zinc-500 line-clamp-1 text'>{descriptionText}</div>
          )}
